...
const getCoordinatesFromAddress = async (address) => {
  ...
  const data = await response.json();
  console.log("📡 카카오 주소 변환 API 응답:", JSON.stringify(data, null, 2));
...
const getNearestPoliceStation = async (lat, lon) => {
  ...
  const data = await response.json();
  console.log("📡 카카오 경찰서 검색 API 응답:", JSON.stringify(data, null, 2));
...
// 🔹 우편번호로 지구대/파출소 찾기
const findNearestPoliceStation = async (zipcode) => {
  try {
    ...
    console.log("✅ 가장 가까운 지구대/파출소:");
    console.log(`이름: ${policeStation.name}`);
    console.log(`주소: ${policeStation.address}`);
    console.log(`전화번호: ${policeStation.phone}`);
...
// 🔹 실행 (테스트할 우편번호 입력)
const police_phone = await findNearestPoliceStation("06236");
console.log(police_phone);
